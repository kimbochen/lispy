...
{
    if (v -> type != LVAL_SEXPR || v -> count < 2) {
        return v;
    }

    for (int i = 0; i < v -> count; i++) {
        lval* child = lval_eval(v -> cell[i]);
    ...
    lval* f = lval_pop(v, 0);
    lval* result = (f -> type == LVAL_SYM) ?
        builtin_op(v, f -> sym) : lval_err("S-expression must start with a symbol");
